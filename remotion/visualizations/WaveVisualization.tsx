...
export const WaveVisualization: React.FC<WaveVisualizationProps> = ({
  ...
  if (!frequencyData || !width || !height) return null;

  const amplitudes = frequencyData
    .slice(0, 0.25 * frequencyData.length)
    .map((v) => processAudioFftValue(v, { maxDb, minDb }));

  const rms = getRMS(amplitudes);
  // Massive amplitude boost to guarantee visibility
  const amplitude = Math.max(height * 0.5, height * rms * 5);
